...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "show_capsules":
        # формируем список кнопок с капсулами
        keyboard = [
            [InlineKeyboardButton(name, callback_data=name)] for name in capsules.keys()
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        # Например: "🌰 Buenos Aires [===4=========]"
        full_name = query.data
        if "[" in full_name and "]" in full_name:
            name_part = full_name.split("[")[0].strip()  # 🌰 Buenos Aires
            intensity_part = "[" + full_name.split("[")[1]  # [===4=========]
        else:
            name_part = full_name
            intensity_part = ""
        ...
        # после вывода капсулы снова показываем кнопку "Показать список"
        keyboard = [[InlineKeyboardButton("📋 Показать список капсул", callback_data="show_capsules")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
